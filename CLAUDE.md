# CLAUDE.md — Shop Price Online Project Instructions

> Проект і тека перейменовані з "Insta-Data"/"Monte-Shop-Price" на **Shop Price Online** (2026-08-29). GitHub-репозиторій лишається `insta-data` (не перейменований).
...
23. **Перший реальний деплой у production** (2026-07-17): git-репозиторій (github.com/SerhiiRiabko/insta-data) до цього моменту мав лише 1 комміт `chore(init)` — увесь Phase 1-4.6 існував тільки локально. Закомічено (2 коміти: backend/Phase 4, frontend+доки/Phase 4) і запушено. Задеплоєно на Hetzner VPS `root@138.199.204.107` — **той самий сервер, що вже хостить hrd-minion** (production Telegram-бот) та kartiq-backend; вибір свідомо підтверджений користувачем після явного попередження про спільну інфраструктуру.

    **Стан сервера на момент деплою:** 3.7GB RAM, **232Мі вільно, 0 swap**, MongoDB не встановлена, порти 3000/3001/3002/8100 вже зайняті (spacecode/postman-script-creator/kartiq-frontend/kartiq-backend). Ризик OOM для існуючих production-сервісів — реальний, не гіпотетичний.

    **Заходи безпеки перед стартом:** (1) додано 2GB swapfile (`/swapfile`, `chmod 600`, у `/etc/fstab`) — чистий, безпечний, оборотний захід; (2) за прямим проханням користувача зупинено (НЕ видалено) `kartiq-backend` (`supervisorctl stop`) і `kartiq-frontend` (`pm2 stop` — виявився в crash-loop, 860+ рестартів) для звільнення пам'яті; відновити: `supervisorctl start kartiq-backend` / `pm2 start kartiq-frontend`.

    **Знайдені й виправлені реальні баги розгортання** (не спрацювали б без спроби реального деплою): `requirements.txt` містив `instagrapi==2.0.0`, який жорстко пінить pydantic<2 і ламає `pip install` всього стеку — пакет ніколи фактично не використовувався (вже давно обгорнутий у try/except, локально теж не встановлений, просто ніхто не намагався встановити requirements.txt з нуля). Видалено разом з `pytesseract`/`pillow` (залежали тільки від вимкненого OCR-пайплайна). Заразом виявлено і виправлено суміжний латентний баг: `app/services/scrapers/__init__.py` імпортував `AromaScraper` (потребує Playwright) БЕЗ try/except, на відміну від решти пакету — якби Playwright колись не встановився, впав би імпорт навіть активного `CijeneScraper`. Обгорнуто тим самим патерном, що й усюди інде в цьому пакеті.

    **Інфраструктура:** MongoDB 7.0 встановлена нативно (не Docker, `apt`, обхід зламаного стороннього apt-джерела cloudflare-warp через явний `sourcelist`), кеш WiredTiger обмежений `cacheSizeGB: 0.25` (дефолт з'їв би ~1.35GB на цій машині). Backend на порту **8010**, фронтенд (production-білд, `next build` + `next start`, НЕ dev-режим) на порту **3010** — обидва обрані вільними (3001/8001 вже зайняті іншими проєктами на цьому сервері). Обидва — під supervisord (`insta-data-backend.conf`/`insta-data-frontend.conf`, той самий патерн, що й `hrd-minion.conf`). `ufw allow 3010/tcp 8010/tcp` — сайт публічно доступний за IP:порт, без домену/SSL (за вибором користувача).

    **Знайдений і виправлений production-специфічний баг:** `auth.py` виставляє `secure=True` на сесійну куку, коли `ENVIRONMENT=="production"` — з доменом/SSL це правильно, але без них (чистий HTTP) браузер просто відкидає таку куку і логін мовчки не працює. Виправлено встановленням `ENVIRONMENT=staging` замість `production` у продакшн `.env` — свідомий, задокументований трейдофф, повернути на `production` одразу після появи домену+SSL.

    **Секрети:** `SECRET_KEY` (JWT) згенерований новий, випадковий (`openssl rand -hex 32`), унікальний для цього деплою. `GROQ_API_KEY` перевикористаний з hrd-minion — **явно дозволено користувачем цього разу** (раніше в сесії свідомо НЕ перевикористовувався без прямого запрошення). Resend НЕ підключений (не обрано користувачем) — тому magic-link лист не надсилається; адмін заведений через email+пароль (`POST /auth/register` + `scripts/bootstrap_admin.py adyvan2008@gmail.com`).

    **Дані:** свіжа порожня БД на сервері заповнена реальним живим скрейпом (`GET /products/matrix-live`, викликаний вручну один раз) — 287 товарів, 798 сирих цін, `name_i18n` заповнився автоматично завдяки сьогоднішньому ingest-хуку (281/287, той самий відсоток, що й локально).

    **Перевірено наскрізно в реальному браузері проти production URL:** `http://138.199.204.107:3010/ukr` — головна сторінка, живий пошук ("молоко" → 11 рядків, коректні заголовки категорій), логін в адмінку через email+пароль, `/ukr/admin` показує реальні дані магазинів. 0 нових помилок консолі (крім очікуваного 401 для неавторизованої сесії). Пам'ять сервера після повного розгортання: ~425Мі вільно + 1.3GB у резерві swap — стабільно, hrd-minion весь час без переривань (uptime не збився).

    **Адреси:**
    - Сайт: http://138.199.204.107:3010/ukr (і /rus /mne /srb /bos /eng)
    - Адмінка: http://138.199.204.107:3010/ukr/admin
    - API: http://138.199.204.107:8010 (Swagger: /docs)
    - Адмін-логін: adyvan2008@gmail.com / пароль згенерований, переданий користувачу окремо (не в цьому файлі)

---

24. **Ребрендинг + фікс липких категорій + відновлення продакшну** (2026-08-29): (1) проект/тека перейменовані з Insta-Data/Monte-Shop-Price на **Shop Price Online** — оновлено README, `<title>`, лого/хедер, next-intl `name` в усіх 6 локалях; GitHub-репо лишився `insta-data`. (2) Знайдено і виправлено реальний баг: `globals.css` мав `overflow-x: hidden` одночасно на `html` і `body` — за специфікацією CSS overflow це перетворює `body` на власний скрол-контейнер і ламає `position: sticky` для БУДЬ-ЯКОГО нащадка відносно вʼюпорта (не тільки категорій — і мобільна sticky-шапка сайту теж не працювала, про що користувач не повідомляв). Прибрано з `body`, залишено тільки на `html`. Додатково додано `position: sticky` на сам рядок/дів категорії (`PriceMatrixLanding.tsx` — `top: 56px` під sticky `<thead>`; `PriceCardsMobile.tsx` — `top: 65px` під sticky nav) — раніше цих стилів там не було взагалі, плашка категорії просто скролилась як звичайний рядок. Перевірено Playwright на localhost (реальні дані, локальний backend + MongoDB) на 1400×900 і 390×844 — категорія коректно залишається закріпленою під час скролу і змінюється на наступну. (3) **Виявлено, що продакшн лежав**: осиротілий `next-server`-процес (pid з 42-денним аптаймом, не під керуванням supervisor) тримав порт 3010, через що керований `insta-data-frontend` падав у `FATAL`/`EADDRINUSE` при кожній спробі рестарту supervisor (`startretries=3` вичерпано, автовідновлення зупинилось). Вбито осиротілий процес, задеплоєно (2) + новий скрипт `backend/scripts/reset_admin_password.py` (bcrypt-скидання пароля — старий пароль ніде не зберігався, non-retrievable), рестарт `insta-data-frontend` — сайт знову `HTTP 200`. **Новий адмін-пароль:** переданий користувачу в чаті, не в цьому файлі (за конвенцією п.23).

25. **Country selector (Phase 1 мультикраїни) + фікс кореневої причини "port EADDRINUSE"** (2026-08-29): (1) `GET /countries` (нове, `countries.py`) — статичний список ME/UA з перекладеними назвами на 6 локалей. (2) `stores` отримали поле `country`; `GET /stores` фільтрує по ньому; адмінка (`AdminPageClient.tsx`) — dropdown країни у формі створення/редагування магазину. (3) `products.py` — усі matrix/by-category ендпоінти приймають `country=ME|UA`, магазини тепер тягнуться динамічно з `db.stores` по країні замість хардкодженого `MOCK_STORES` (Чорногорія-only); товари тегуються `country` при персисті; для UA (де скрейпера ще нема) — чесний порожній список магазинів + стан "даних ще нема" замість чорногорського mock під чужим прапором. (4) Міграція `backend/scripts/seed_ua_country_support.py` — бекфіл `country=ME` на існуючі 4 магазини + 290 товарів, сідінг 6 українських магазинів (АТБ, Сільпо, Фора, Novus, Varus, Коло) без скрейпера, тільки список. (5) Фронтенд — `<select>` країни поруч з мовним (persist у `localStorage`), прокинутий у `StoresModal` і фетч матриці цін. Перевірено Playwright локально і на проді: перемикання ME↔UA міняє колонки магазинів, показує "Поки немає даних по цій країні" для UA, адмінка редагує країну магазину.
    **Знайдена і виправлена корінна причина повторюваного "порт зайнятий" бага** (та сама проблема з п.24, стався ЗНОВУ під час цього деплою): `insta-data-frontend.conf` мав `command=npm run start` без `stopasgroup`/`killasgroup` — supervisor зупиняє тільки `npm`-обгортку, а дочірній `next-server`-процес переживає SIGTERM і лишається сиротою, тримаючи порт 3010 назавжди (до ручного `kill -9`). Додано `stopasgroup=true` + `killasgroup=true` в конфіг (supervisor тепер сигналить всій групі процесів) — має остаточно прибрати цей клас багів при майбутніх рестартах/деплоях.

26. **Реальні скрапери України (Phase 3): Сільпо + Varus живі, підсвітка акцій** (2026-08-29): Розвідка 6 сайтів показала: Сільпо і Varus — жодного антибот-захисту, чисті селектори; АТБ — за Cloudflare managed challenge; Novus — каталог відкривається лише після вибору адреси доставки (не досліджено глибше); Коло — взагалі немає онлайн-каталогу/цін (лише промо-картинки й адреси магазинів, WordPress-сайт); Фора — CSR React SPA, теж не досліджено глибше цього разу.
    - `silpo_scraper.py`, `varus_scraper.py` — Playwright-based (звичайний `aiohttp` GET повертає 403 на обох, найімовірніше TLS/JA3-фінгерпринт-перевірка, а не перевірка заголовків, бо жодної Cloudflare-заглушки не показується). Парсять ~10 обраних продуктових категорій кожен, витягують назву/ціну/стару_ціну/одиницю з підтверджених DOM-селекторів (aria-label у Сільпо містить "стара ціна X, знижка Y%, нова ціна Z" одним рядком).
    - `atb_scraper.py` — написаний і Playwright-based, але **не зареєстрований** в оркестраторі: сайт має справжній Cloudflare managed challenge, який не пройшов жоден із випробуваних підходів (патч `navigator.webdriver`, headed-режим, справжній встановлений Chrome через `channel="chrome"`) — жоден не пройшов виклик навіть за 18с. Лишено вимкненим з коментарем-поясненням; код готовий до реєстрації, якщо колись з'явиться робочий обхід (stealth-бібліотека або платний unlocker-сервіс).
    - `orchestrator.py` — скрапери тепер тегуються країною (`scraper_country`), `run_all(country=)` запускає лише релевантні для вкладки скрапери.
    - `base_scraper.py` — `ScrapedProduct` отримав `old_price`/`is_promo`/`unit`.
    - `product_matcher.py` — групи тепер тримають `promo_by_store`; **знайдено і виправлено реальний баг**: `category.title()` калічить будь-яку категорію з апострофом (`м'ясо` → `М'Ясо`, велика Я) — замінено на капіталізацію лише першої літери.
    - `category_map.py` — `classify_group_category` була Чорногорія-only (мапила сербські/чорногорські назви cijene.me, все інше падало в "Інше") — **буквально кожна українська категорія падала в "Інше"**, поки це не виправлено (тепер пропускає нерозпізнані як Монтенегро категорії напряму, замість відкидання). Додано "Сири"/"Хлібобулочні вироби" в `CATEGORY_ORDER` (backend+frontend); категорії 3 скраперів вирівняні під існуючу таксономію (Молочка, М'ясо і риба, Солодощі та снеки) замість вигаданих дублікатів.
    - `products.py` — `/matrix-live` і `/by-category` більше не блокують UA заглушкою — реально запускають українські скрапери.
    - Фронтенд: акційні комірки підсвічуються зеленим (окремий відтінок від "найдешевше") незалежно від того, чи товар найдешевший — новий `promo: boolean[]` на `MatrixProduct`. `formatPrice()` була захардкоджена на €/EUR — Україна показувала гривневі ціни зі знаком євро; тепер приймає `country`, показує ₴ для UA. Таймаут `priceMatrixLive`/`byCategory` піднято з 30с до 150с (Playwright-скрапінг ~10 категорій × 2 магазини без спільного агрегатора займає ~90-120с, старий таймаут обрізав кожен UA-рефреш клієнтською стороною, хоча сам скрап і персист продовжували виконуватись на сервері).
    - **Деплой на прод:** зламане apt-джерело `cloudflare-warp` (той самий 404, що й раніше для MongoDB) блокувало `playwright install --with-deps` — вимкнено (`.list` → `.list.disabled`), системні залежності виявились вже встановлені, Chromium довантажено. Реальний скрап на проді: 447 товарів (Сільпо+Varus), перевірено в браузері на `http://138.199.204.107:3010/ukr` — реальні ₴-ціни, коректна категорія "ОВОЧІ · 40", підсвітка найдешевшого. Пам'ять сервера стабільна (1.4Gi вільно, hrd-minion не постраждав), 0 осиротілих Playwright-процесів після скрапу (`stopasgroup`/`killasgroup` з п.25 тримає).
    - Перевірено наскрізно локально й на проді: ~445-447 згрупованих товарів, 8 коректних категорій, 217 позначені як акційні.

27. **Фора + Novus (4 живих UA-джерела разом)** (2026-08-29): `fora_scraper.py` (CSR React, Playwright, промо через `.isPromo`/`.old-price`/`.discount-count`) і `novus_scraper.py` (zakaz.ua-платформа, React SPA — знахідка: категорії `/uk/categories/<slug>/` рендерять реальні товари й ціни БЕЗ вибору адреси доставки, хоча платформа зазвичай прив'язує каталог до адреси; поки що це трактується як каталог за замовчуванням/Київ, без розрізу по містах). Обидва зареєстровані в оркестраторі поруч із Сільпо/Varus.
    Перевірено локально й на проді з усіма 4 джерелами одночасно (Novus+Varus+Сільпо+Фора): ~804-849 згрупованих товарів, 9 коректних категорій, 287-288 акційних. **Пам'ять під час паралельного скрапу 4 магазинів** просідала до 146Mi вільно (з 1.4-1.5Gi) — тимчасово, повністю відновлювалась одразу після завершення скрапу (1.5-2.1Gi вільно), hrd-minion не постраждав. Вартий уваги момент на майбутнє: якщо додавати ще джерела або автоматичний щотижневий UA-рефреш, варто або обмежити паралелізм оркестратора, або рознести запуски в часі — 3.7Gi RAM сервера не безмежні.
    Відома дрібна проблема: зображення товарів з Фори (`content.fora.ua`) не завантажуються в браузері (`ERR_HTTP2_PROTOCOL_ERROR`, ймовірно hotlink-захист) — не полагоджено, косметичний дефект, не блокує функціонал.

28. **Фікс міжмагазинного зіставлення товарів — майже нічого не збігалось** (2026-08-29): користувач поскаржився, що таблиця цін виглядає "розрізненою" — майже кожен рядок мав ціну лише в одній колонці. Перевірка підтвердила: 5 зіставлень з ~800 товарів. Дві корінні причини: (1) категорія — частина ключа зіставлення в `product_matcher.py`, а Novus сам групує "Фрукти та овочі" одним кошиком, тоді як Сільпо/Varus/Фора розділяють — однакові назви товарів НІКОЛИ не збігались лише через різні рядки категорії. Виправлено: `category_map.py` отримав `split_ua_produce_category()` + `VEGETABLE_KEYWORDS_UA`/`FRUIT_KEYWORDS_UA` (той самий патерн, що вже був для чорногорської "Voće i povrće"), Novus тепер розбиває свою категорію по ключових словах перед збереженням. (2) Regex відрізання одиниці виміру в `_extract_name_and_unit` розпізнавав лише латинські літери (`[a-z]+`) — кириличні "кг"/"г"/"л" ніколи не відрізались, тож "Картопля біла, кг" (Фора) і "Картопля біла" (Novus, вага в окремому полі) вважались різними товарами. Додано `CYRILLIC_TRAILING_UNIT_RE`.
    Перевірено: зіставлення зросли з 5 до 33-44 (залежно від скану) — реальні коректні збіги штибу "Молоко «Галичина» ультрапастеризоване 2,5%" одразу в 4 магазинах, "Банан" в Novus+Фора. Задеплоєно, кеш на проді оновлено живим скрапом. Більшість товарів все одно НЕ збігаються — це очікувано і коректно: незалежні мережі здебільшого продають різні конкретні бренди/SKU, а не спільний каталог (на відміну від Чорногорії, де cijene.me — це вже готовий агрегатор).

29. **Сортування порівнянних товарів нагору + синоніми (томат/помідор, лосось/сьомга)** (2026-08-29-30): (1) навіть після фіксу зіставлення реальні порівняння (32-44 товари) губились серед сотень одиничних рядків — `groupByCategory()` (`productMatrix.ts`) тепер сортує рядки кожної категорії за кількістю магазинів з ціною (спадання), стабільне сортування зберігає порядок серед рівних. (2) Користувач помітив на сайті АТБ, що "томати" — це те саме, що "помідор" деінде; обійшов усі 5 магазинів вручну (браузером) — АТБ каже "томати", Сільпо "томат", Novus/Varus/Фора "помідор"; АТБ також має окремо "лосось" і "сьомга", хоча це та сама риба. Додано `NAME_SYNONYMS` в `product_matcher.py` (root-based regex, нечутливий до відмінків) — нормалізує всі форми до одного канонічного слова перед побудовою ключа зіставлення. Серед 4 активних UA-скраперів назви помідорів вже в однині граматично, тож синонім одразу дав нові збіги: "Томат" тепер порівнюється в 3 магазинах (Novus/Сільпо/Фора) замість 1. АТБ (переважно множина "томати") не вирівняється, поки не буде реактивований — некритично, він і так вимкнений.
    Кожна зміна логіки зіставлення змінює `id` товару (хеш від name+category) — старі документи в Mongo лишаються сиротами. Після кожного такого фіксу довелось чистити `db.products` (`{"country": "UA"}`) і робити свіжий скрап, щоб кеш на проді не накопичував дублікати. Варто пам'ятати про це і надалі.

---

30. **Фікс "зникаючих" цін + UA-варіант hero + пагінація + 4 нові полиці + фікс цін "за 100г"** (2026-08-30): (1) Користувач: "коли перевантажую сторінку, бачу ціни для усіх магазинів, а потім вони зникають". Причина: `LandingPageDesignBrief.tsx` стартував з `country='ME'` за замовчуванням, робив запит, і ЛИШЕ ПОТІМ ефект підвантажував справжню країну з localStorage і перезапускав фетч — якщо збережена країна була UA, користувач бачив спалах повних (щільних) даних Чорногорії, який миттєво замінювався на рідші дані України. Виправлено: новий стан `countryReady`, фетч чекає, поки localStorage прочитано, перед першим запитом. (2) При виборі України hero-блок тепер показує фото українського пшеничного поля (Wikimedia Commons, `Wheat_fields_in_Ukraine-5961.jpg`) і текст "Порівняння цін на продукти в супермаркетах України" замість чорногорських за замовчуванням.
    (3) **Пагінація**: всі 4 живі UA-скрапери читали лише першу сторінку кожної категорії, хоча на сайтах є 2-14 сторінок. Знайдено схему пагінації кожного магазина (`?page=N` у Varus/Сільпо/Novus, `?to=N&from=N` у Фора) і додано прохід до 2 сторінок/категорію з відновленням сторінки при краху (`page.is_closed()` → новий `new_page()`). (4) **4 нові товарні полиці** на кожному скрапері: Бакалія, Консервація, Заморожені продукти, Ковбаси/делікатеси — раніше не скрапились ЗОВСІМ. (5) Побічно знайдено і виправлено реальний ціновий баг: Varus і Сільпо показують вагове м'ясо/делікатеси/овочі-фрукти з позначкою "за 100 г" замість "за 1 кг" — сирий скрапований номер був у 10 разів меншим за реальну ціну за кг (кавун "за 2 грн" замість ~20 грн/кг). Виправлено множенням на 10 для категорій Овочі/Фрукти/М'ясо і риба (для Varus — за явним текстом "за 100 г" на картці; для Сільпо — коли вага рівно "100г" саме в цих категоріях, бо в Молочці/Сирах "100г" — це реальна вага фасованого товару, а не тарифна одиниця).

31. **Зіставлення за базовою назвою для овочів/фруктів/м'яса (свідома втрата точності заради ширшого покриття)** (2026-08-30): користувач прямо попросив: "по овочах, фруктах і м'ясу орієнтуватися на саме назву, а не додаткові слова". Додано `_simplify_core_name()` в `product_matcher.py`: для Овочі/Фрукти/Фрукти та овочі canonical-назва обрізається до ПЕРШОГО слова ("Яблуко Гала Роял вагове" → "яблуко") — сорт/бренд/походження ігноруються повністю. Для М'ясо і риба (де порядок слів непослідовний між магазинами) — токени фільтруються за списком `MEAT_CORE_WORDS` (типи тварин, відруби), решта (бренд, "фермерське", "домашнє") відкидається. Результат: "Яблуко"/"Кавун"/"Банан" тепер збігаються одразу в 4-5 магазинах замість одного. Явний trade-off: різні сорти одного фрукту тепер показуються в одному рядку — user-approved.

32. **АТБ: розвідка обходу Cloudflare (patchright, headed-режими) + ручний збір даних як тимчасовий міст** (2026-08-30): за прямою вказівкою користувача "спробуй патчі/обходи" протестовано (1) повний stealth JS-патч (`navigator.webdriver`, plugins, chrome.runtime тощо) — заблоковано за ~3с; (2) `patchright` (CDP-патчений форк Playwright спеціально проти такого детекту) headless і headed — теж заблоковано. Обидва підтвердили: захист спрацьовує ДО виконання JS, отже це TLS/мережевий фінгерпринт, а не JS-виклик. Водночас MCP Playwright-браузер (реальна взаємодія в цій сесії) Cloudflare проходив стабільно. Тому дані АТБ по 11 категоріях (~360 товарів) зібрано вручну через цю сесію і залито в базу скриптом `backend/scripts/merge_atb_manual.py` (перевикористовує `ProductMatcherService` — та сама нормалізація, що й у живих скраперів), дані лежать у `backend/scripts/data/atb_raw.jsonl`. Одночасно виявлено: Сільпо після кількох десятків повторних скрапів за сесію почав відповідати таймаутом на КОЖНУ категорію з IP сервера (при цьому з іншого IP — працює нормально) — ймовірний тимчасовий rate-limit від самого сайту, не баг коду. Той самий підхід (ручний збір + `merge_silpo_manual.py`, перевикористовує `SilpoScraper._parse_item` — включно з фіксом "за 100г") відновив Сільпо з 20 до ~247 товарів.

33. **АТБ — реальна автоматизація через undetected-chromedriver + Xvfb** (2026-08-30): користувач підказав спробувати Selenium. Виявилось: `undetected-chromedriver` у **headless**-режимі так само блокується, але у **звичайному (headed) режимі** Cloudflare проходить стабільно щоразу за ~3-10с — headless сам по собі є ще одним детектованим сигналом, не лише сама автоматизація. Сервер без монітора, тому піднято постійний віртуальний дисплей: новий supervisor-сервіс `xvfb.conf` (`Xvfb :99 -screen 0 1366x768x24`) + `DISPLAY=":99"` у середовищі `insta-data-backend.conf`. `atb_scraper.py` переписано: метод `scrape_with_playwright()` (назва лишена для сумісності з `BaseScraper`) тепер запускає Selenium через `asyncio.to_thread` (синхронний виклик, щоб не блокувати event loop), керуючи `/usr/bin/chromium-browser` (snap, версія 151, пінена явно — патчер `undetected-chromedriver` не вміє парсити snap-версію автоматично). Зареєстровано в `orchestrator.py`. Пагінації на АТБ не знайдено (сторінка стабільно віддає ~25-36 товарів, не росте при скролі) — категорії лишились без сторінок. `setuptools<81` доданий в requirements.txt (потрібен, бо `undetected-chromedriver` досі імпортує видалений з stdlib `distutils`). Наживо: 410+ товарів АТБ у повному живому скрапі.
    **Побічний, важливий архітектурний фікс**: `_persist_live_products()` раніше повністю ПЕРЕЗАПИСУВАЛА масив `prices` при кожному живому скрапі — якщо один магазин цього разу не відповів (як щойно сталось із Сільпо), його ціни стирались З УСІХ товарів, хоча магазин продовжує їх продавати. Тепер функція зливає нові ціни з наявним документом: якщо `prices[i] is None` у свіжому скрапі, а в базі є старе значення — старе зберігається. Потребує `stores` (у тому ж порядку, що масив цін) для перерахунку `cheapest_store` після злиття; таймаут підняли з 5с до 60с (злиття додає один `find_one` на товар, а каталог України — 1500+ позицій, але це фоновий `asyncio.create_task`, тож не блокує HTTP-відповідь).
    Яйця отримали окрему категорію "Яйця" замість "Молочка" (за проханням користувача) — розділення за ключовим словом у назві, той самий патерн, що і Овочі/Фрукти.

34. **Категорія "Алкоголь"** (2026-08-30): жоден з 5 магазинів не мав посилання на алкоголь у головному меню (ймовірно, через законодавчі обмеження продажу онлайн) — знайдено реальні URL через хлібні крихти товарів: Varus `/alkogol`, Сільпо `/category/alkogol-22`, Фора `/category/alkogol-2451`, Novus `/categories/eighteen-plus/` (без вікового підтвердження для автоматичного доступу), АТБ `/catalog/292-alkogol-i-tyutyun` (змішано з тютюном — прийнято як є). ~171-225 товарів алкоголю в живому скрапі.

35. **Дитячі товари, гігієна, побутова хімія, зоотовари + нормалізація множини овочів/фруктів** (2026-08-30): всі скрапери охоплювали лише продукти харчування — цілі товарні групи не скрапились взагалі. Знайдено реальні URL на всіх 5 сайтах (Varus `/tovari-dlya-ditey`, `/kosmetika-ta-doglyad`, `/pobutova-himiya`, `/tovari-dlya-tvarin`; аналогічно для Сільпо/Фора/Novus/АТБ). Додано нову категорію "Зоотовари" в `CATEGORY_ORDER` (backend+frontend). Окремо користувач помітив: "кабачок і кабачки те саме, огірки і огірок також" — оскільки `_simplify_core_name` для овочів/фруктів бере лише ПЕРШЕ слово, множина й однина ("Огірки" АТБ проти "Огірок" деінде) ніколи не збігались через українське чергування голосних (огірок→огірки: "о" випадає, просте відрізання суфікса не відновлює однину). Додано явні пари в `NAME_SYNONYMS` (той самий патерн, що й томат/лосось) для найпоширеніших овочів/фруктів: огірок, кабачок, баклажан, буряк, перець, цибуля, морква, картопля, капуста, яблуко, груша, кавун, диня. Перевірено: базовий рядок "Огірок" тепер збирає ціни з 4 магазинів замість 1-2.
    Після очищення `db.products` для свіжого перерахунку (нова логіка зіставлення) Сільпо знову впав до 20 товарів (rate-limit ще не минув повністю) — `merge_silpo_manual.py` перезапущено, відновлено до 246.

36. **Фікс обрізання `/matrix-cached` на 2000 товарів + автооновлення для України щопонеділка** (2026-08-30): за прямим запитом користувача перевірено повноту даних — виявлено, що `db.products.find(...).to_list(2000)` в `/matrix-cached` (ліміт, виставлений ще коли товарів було ~290) мовчки обрізав ~16% реального каталогу України (2371 товар у базі, показувалось 2000). Піднято до 10000. Окремо виявлено: `refresh_prices_job()` був жорстко прописаний лише на Чорногорію — **Україна взагалі ніколи не оновлювалась автоматично**, лише вручну кнопкою "Оновити ціни". Функція узагальнена (`country` параметр), заплановано ДВА завдання в `main.py`: Чорногорія понеділок 03:00 Київ, Україна 03:15 (щоб не змагались за CPU/пам'ять одночасно — UA займає 5-6 хв проти секунд/хвилини у ME) — за прямим проханням користувача перенесено з 07:00 у вікно "3 ранку — 6-7 ранку".
    **Підсумковий стан бази України на кінець сесії:** 3110 товарів, 141 порівняння у 2+ магазинах, 6 магазинів (Коло — підтверджено вкотре: реального каталогу з цінами немає).

37. **Фікс зіставлення напоїв (Pepsi та інші), без ламання розрізнення газованої/негазованої води** (2026-08-30): користувач: "чому пепсі тільки в одному магазині". Причина: кожен магазин пише один і той самий Pepsi по-своєму — "Напій газований Pepsi" (Novus), "Напій Pepsi сильногазований" (Varus), "Напій Pepsi Пепсі-Кола безалкогольний сильногазований" (Фора) — набори слів відрізнялись, тож навіть нечутливе до порядку зіставлення не рятувало. Додано до `FILLER_WORDS_RE`: "напій"/"напої"/"безалкогольний" (завжди прибираються — ніколи не відрізняють один напій від іншого) + кириличний повтор бренду "Пепсі-Кола"/"Пепсі" (прибирається як зайве дублювання, а не підмінюється — підміна створювала б ДРУГИЙ токен "pepsi" і псувала збіг). Слова газації ("газований"/"сильногазований"/"негазований") — окремий `CARBONATION_WORDS_RE`, прибирається лише коли в назві немає кореня "вод" (для мінеральної води газована/негазована — це реально різні товари з різною ціною, "Вода Моршинська негазована" не повинна зливатись з "...сильногазована"). Перевірено в обидва боки: Pepsi тепер у 3 магазинах (було 1), Моршинська негазована/сильногазована лишились коректно розділеними.

38. **Пошук цін для "Коло" — третя (і четверта) незалежна перевірка + реальне джерело знайдено** (2026-08-31): користувач наполегливо просив ще раз пошукати. Перевірено `gotoshop.ua/kiev/shops/kolo/catalog/` — прямим текстом "Акційний каталог цього магазину наразі відсутній" (0 каталогів), той самий висновок, що й раніше. За прямим посиланням від користувача перевірено й **знайдено робоче джерело**: `ukrtopshop.com/kiev-kolomarket.html` хостить скановані Instagram-story-подібні промо-картинки окремих кампаній Коло (не HTML-текст — ціни намальовані на зображенні). З останньої актуальної кампанії ("Акції з 06.08.2026", 6 картинок) вручну прочитано (переглянуто скріншотами) 7 реальних товарів з цінами (Три Ведмеді Морозиво, Pirulo, Живчик, OSHEE, Tea Moments, Milka тощо). Обсяг малий і принципово не автоматизований (потрібен OCR, якого в проєкті свідомо немає — `pytesseract`/`pillow` вже раз прибирались як мертвий вантаж від закинутого Instagram-OCR пайплайна), але це реальні, перевірені дані, а не порожня колонка. Дані лежать у `backend/scripts/data/kolo_raw.jsonl`, залиті скриптом `merge_kolo_manual.py` (той самий патерн, що й `merge_atb_manual.py`/`merge_silpo_manual.py`). Перевірено на живому сайті: "Живчик Яблуко" — 22,00₴ у колонці Коло, позначено найдешевшим.

---

**Остання оновлена:** 2026-08-31
**Автор:** Serhii Riabko